...
const FIXTURE_PASSWORD =
  process.env.ANALYTICS_FIXTURE_PASSWORD ?? 'Validation123!';
...
async function main() {
  ...
  if (!category || !contract || !shift || !teacherPosition || !headPosition) {
    throw new Error(
      'Required teacher catalogs are missing. Run the seed first.',
    );
  }
  ...
  const department1 = await prisma.department.upsert({
    ...
    create: {
      id: ids.department1,
      name: 'AV Departamento Principal',
  ...
  const department2 = await prisma.department.upsert({
    ...
    create: {
      id: ids.department2,
      name: 'AV Departamento Secundario',
      facultyId: faculty.id,
    },
  });
  ...
  const virtualRoomType =
    ...
    (await prisma.roomType.create({
      data: { description: 'Espacio Virtual' },
    }));
  ...
  const virtualModality =
    (await prisma.modality.findUnique({
      where: { name: 'Espacio Virtual' },
    })) ??
  ...
  const monitorType = await prisma.monitorType.findFirst({
    orderBy: { description: 'asc' },
  });
  const monitorSize = await prisma.monitorSize.findFirst({
    orderBy: { description: 'asc' },
  });
  const pcType = await prisma.pcType.findFirst({
    orderBy: { description: 'asc' },
  });
  if (!brand || !condition || !monitorType || !monitorSize || !pcType) {
    throw new Error(
      'Required inventory catalogs are missing. Run the seed first.',
    );
  }
  ...
  const role = await prisma.role.upsert({
    where: { id: ids.role },
    update: {
      name: 'AV_ANALYTICS_READER',
      description: 'Validation reader',
      isSuperAdmin: false,
    },
    create: {
      id: ids.role,
  ...
  const users = [
    ...
    [
      ids.userTeacher3,
      'AV Docente Tres',
      'av-teacher-3@example.invalid',
      'AV-T3',
      true,
    ],
    [
      ids.userCoordinator1,
      'AV Coordinador Uno',
      'av-coordinator-1@example.invalid',
      'AV-COORD-1',
      true,
    ],
    [
      ids.userCoordinator2,
      'AV Coordinador Dos',
      'av-coordinator-2@example.invalid',
      'AV-COORD-2',
      true,
    ],
    [
  ...
  for (const [id, teacherId, positionId, centerDepartmentId] of [
    ...
    [
      ...
      ids.teacherCoordinator1,
      headPosition.id,
      centerDepartment1.id,
    ],
    [
      'b1000000-0000-4000-8000-000000000005',
      ids.teacherCoordinator2,
      headPosition.id,
      centerDepartment2.id,
    ],
  ] as const) {
  ...
  for (const [id, teacherId, periodId] of [
    ...
    await prisma.academicAssignmentReport.upsert({
      ...
      create: {
        ...
        teacherId,
        periodId,
        centerDepartmentId: centerDepartment1.id,
      },
    });
  }
  ...
  for (const [
    ...
    const modalityId =
      modalityName === 'Espacio Virtual'
        ? virtualModality.id
        : presentialModality.id;
  ...
  const activityType2 = await prisma.activityType.upsert({
    ...
    create: {
      name: 'AV Investigación',
      description: 'AV activity type research',
    },
  });
  for (const [
    id,
  ...
  const counts = {
    periods: await prisma.academicPeriod.count({
      where: { id: { in: [period1.id, period2.id, periodZero.id] } },
    }),
    classrooms: await prisma.classroom.count({
      where: { id: { in: classroomData.map(({ id }) => id) } },
    }),
    sectionsP1: await prisma.courseClassroom.count({
      where: {
        id: {
          in: [
            ids.section1,
    ...
    sectionsP2: await prisma.courseClassroom.count({
      where: { id: { in: [ids.section6, ids.section7] } },
    }),
    activities: await prisma.complementaryActivity.count({
      where: { id: { in: [ids.activity1, ids.activity2, ids.activity3] } },
    }),
    checks: await prisma.scheduleComplianceCheck.count({
      where: { id: { in: [ids.check1, ids.check2, ids.check3, ids.check4] } },
    }),
  };
  const manifest = {
    ...
    passwordPolicy:
      'Generated fixture users share ANALYTICS_FIXTURE_PASSWORD; value is intentionally not stored.',
    ids: {
      center: center.id,
    ...
    expectedP1: {
      ...
      classroomCapacity: {
        ...
        capacityDataCoverage: 66.6666666667,
      },
      technology: {
        eligibleClassrooms: 3,
