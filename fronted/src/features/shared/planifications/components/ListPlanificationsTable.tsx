...
import { DataTable, IDataTableColumn, Pagination, TagError } from '@components';
import { TCurrentAcademicPeriod } from '@features/teachers';
		...
		},
		{
			...
			render: (row: TPlanificationRow) => (
				<Link
					...
					to="/coordinadores/datos-planificacion"
					state={{
						year: row.year,
						pac: row.pac,
						periodId: row.id,
