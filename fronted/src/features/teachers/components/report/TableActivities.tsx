...
import { ComplementaryActivity } from '../form';
import {
	EActivityType,
	activityRegistrationStatus,
} from '@features/teachers/constants';
import { TComplementaryActivity } from '@features/teachers/types';
import { PencilIcon, TrashIcon } from '@heroicons/react/24/outline';
import { useDeleteComplementaryActivity } from '@features/teachers/hooks';
				...
				<table className="w-full min-w-[700px] text-sm">
						<thead className="bg-[#144C74] text-white">
							...
							<tr>
								...
								{activityRegistrationStatus[activityType] && (
									<>
										<th className="py-3 px-4 font-semibold">Si</th>
										<th className="py-3 px-4 font-semibold">No</th>
										<th className="py-3 px-4 font-semibold">
											No. de Expediente
										</th>
								...
								<th className="py-3 px-4 font-semibold">Nivel de avance</th>
								<th className="py-3 px-4 font-semibold">
						...
						<tbody className="text-center text-sm text-foreground [&>tr:nth-child(odd)]:bg-white [&>tr:nth-child(even)]:bg-gray-100 hover:bg-gray-200 transition-colors duration-150">
							{activities.map((ac, index) => (
								<tr id={ac.id} key={ac.id}>
									...
									<td className="py-3 px-4">{ac.progressLevel}</td>
									<td className="py-3 px-4">
