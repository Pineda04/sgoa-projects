...
import { TUpdateUser, userUpdateSchema } from '../schemas';
...
} from '@features/shared/academic';
import { Error, Loading } from '@components';
import { useChangeStatusActiveStatus, useUpdateUser } from '../hooks';
...
// 	type: TField;
// 	readOnly?: boolean; // NOTE: Por si se desea colocar información no editable
// 	element?: React.ReactNode;
// 	handleChange: () => void;
// 	handleBlur: {
...
export const UserView = ({ initialData, isModal }: IProps) => {
	...
	// 		cat => cat.id === userFormik.values.categoryId
	// 	);
	//
	// 	if (!category) return;
	//
	...
	return (
		<div className={`${!isModal ? 'min-h-screen ' : ''}px-4 py-4`}>
			<div
				className={`sticky z-20 mb-5 ${isModal ? 'bg-white' : 'bg-gray-50 top-[57px]'}`}
			>
