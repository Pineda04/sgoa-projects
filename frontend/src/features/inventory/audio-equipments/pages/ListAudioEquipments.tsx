...
export const ListAudioEquipments = () => {
	...
	const [selectedEquipment, setSelectedEquipment] =
		useState<TAudioEquipment | null>(null);

	const ability = useAbility();
	const canCreate = ability.can('create', 'audio-equipments');
	const canUpdate = ability.can('update', 'audio-equipments');
	const canDelete = ability.can('delete', 'audio-equipments');

	// 2. Consumo de Queries y Mutaciones
	const { data: audioResponse, isLoading } = useGetAllAudioEquipments();
	const { mutate: deleteAudioEquipment, isPending: isDeleting } =
		useDeleteAudioEquipmentMutation();
			...
			<div className="flex justify-between items-end mb-5">
				<div>
					...
					<p className="text-muted-foreground mt-1">
					  Tipos de equipos de audio disponibles en la institución.
					</p>
				</div>
				{canCreate && (
					<Button
