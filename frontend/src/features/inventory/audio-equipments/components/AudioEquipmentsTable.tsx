...
export const AudioEquipmentsTable = ({
	...
	const [selectedAudioEquipment, setSelectedAudioEquipment] = useState<
		TAudioEquipment | undefined
	>();

	const [isEditOpen, openEdit, closeEdit] = useModal();
	const [editingAudioEquipmentId, setEditingAudioEquipmentId] = useState<
		string | null
	>(null);
	...
	const handleConfirmDelete = async () => {
		...
		try {
			await deleteAudioEquipmentMutation.mutateAsync(
				selectedAudioEquipment.id
			);
			handleCloseDelete();
		} catch {
			// El error ya es manejado por React Query / notificaciones globales
		}
	};
