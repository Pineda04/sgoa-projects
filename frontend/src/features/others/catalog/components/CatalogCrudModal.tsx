...
export const CatalogCrudModal = ({
  ...
  const handleSave = async () => {
    ...
    const updateItems = items
      .filter(
        (i) => i.id && i.value !== originalMap.get(i.id!)?.value
      )
    ...
    if (createItems.length === 0 && updateItems.length === 0 && deleteIds.length === 0) {
      return;
    }
      ...
      <div className="flex flex-col h-full">
        ...
        <div className="flex-1 min-h-0">
                    ...
                    <div className="flex-1 min-w-0">
                      {isEditing ? (
                        ...
                        <span className="block text-sm text-slate-700 px-2 py-1">
                          {item.value || <span className="text-gray-300">Valor vacío</span>}
                        </span>
                      )}
                    </div>
                    <button
          ...
          <Button
            ...
            size="sm"
            onClick={handleAdd}
            className="mt-3 w-full border-dashed border-green-400 text-green-600 hover:text-white hover:bg-green-600 hover:border-green-600"
          >
