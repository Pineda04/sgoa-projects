...
export const DegreeFormModal = ({
    ...
    const { mutateAsync: createUndergrad, isPending: isPendingCreateUndergrad } = useCreateUndergrad();
    const { updateUndergrad, isPendingUpdate: isPendingUpdateUndergrad } = useUpdateUndergrad(degree?.id ?? '');
    const { mutateAsync: createPostgrad, isPending: isPendingCreatePostgrad } = useCreatePostgrad();
    const { updatePostgrad, isPendingUpdate: isPendingUpdatePostgrad } = useUpdatePostgrad(degree?.id ?? '');
        ...
        <ModalBase isOpen={isOpen} onClose={onClose}>
            <div className="p-2">
                <h1 className="text-xl font-bold text-slate-800 mb-1">
                    {isEdit ? `Editar ${label.singular}` : `Crear Nuevo ${label.singular}`}
                </h1>
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                    ...
                    <div className="flex justify-end gap-3 pt-4 border-t border-gray-100">
                        ...
                        >
                            ...
                            <span>
                                {isPending ? 'Guardando...' : isEdit ? `Actualizar ${label.singular}` : `Guardar ${label.singular}`}
                            </span>
                        </Button>
                    </div>
                </form>
