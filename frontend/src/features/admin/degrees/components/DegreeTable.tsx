...
const LABELS: Record<DegreeType, { singular: string; emptyMsg: string }> = {
	undergrad: {
		singular: 'pregrado',
		emptyMsg: 'No hay pregrados registrados',
	},
	postgrad: {
		singular: 'posgrado',
		emptyMsg: 'No hay posgrados registrados',
	},
};
...
export const DegreeTable = ({
	...
	const [selectedDegree, setSelectedDegree] = useState<
		TAcademicCommonProps | undefined
	>();

	const { deleteUndergrad, isPendingDelete: isPendingDeleteUndergrad } =
		useDeleteUndergrad(selectedDegree?.id ?? '');
	const { deletePostgrad, isPendingDelete: isPendingDeletePostgrad } =
		useDeletePostgrad(selectedDegree?.id ?? '');
