...
const SORTS = [
	...
	{ value: 'maxCapacity:desc', label: 'Mayor capacidad' },
	{ value: 'maxCapacity:asc', label: 'Menor capacidad' },
	{ value: 'capacityStatus:asc', label: 'Estado ascendente' },
	{ value: 'capacityStatus:desc', label: 'Estado descendente' },
] satisfies { value: ClassroomCapacitySort; label: string }[];

const CapacityStatus = ({ row }: { row: ClassroomCapacityRow }) => {
	if (row.capacityStatus === 'known')
		return (
			<span className="inline-flex items-center gap-1 rounded-full border border-primary/30 bg-primary-light px-2 py-1 text-xs font-semibold text-primary">
				<CheckCircle2 className="size-3" />
				Conocida
			</span>
		);
	if (row.capacityStatus === 'invalid')
		return (
	...
	return (
		<span className="inline-flex items-center gap-1 rounded-full border border-border bg-muted px-2 py-1 text-xs font-semibold text-muted-foreground">
...
const COLUMNS = [
	...
	{ key: 'roomType', header: 'Tipo de aula' },
	{
		...
		render: (row: ClassroomCapacityRow) =>
			row.maxCapacity ?? 'Sin información',
	},
	{
		key: 'capacityStatus',
		header: 'Estado',
		render: (row: ClassroomCapacityRow) => <CapacityStatus row={row} />,
	},
...
export const ClassroomCapacitySection = () => {
	...
	const details = useClassroomCapacityDetails(
		classroomCapacityFilters
			? {
					...classroomCapacityFilters,
	...
	const exportRows = async () => {
		setIsExporting(true);
		try {
			const response = await analyticsApi.exportClassroomCapacityDetails({
				...classroomCapacityFilters,
				metric: 'installed_capacity',
				sort: capacitySort,
			});
			downloadBlob(
				response.data,
				`analytics-capacidad-aulas-${classroomCapacityFilters.periodId}.xlsx`
			);
		} catch {
			await genericAlert(
				'No fue posible exportar la capacidad instalada.',
				ESwalIcons.ERROR
			);
		} finally {
		...
		<div>
			<div className="mb-5">
				...
				<p className="mt-1 text-sm text-muted-foreground">
					Capacidad y calidad de datos del catálogo actual de aulas.
				</p>
			</div>
			{summary.isError ? (
				<p className="rounded-xl border border-destructive/30 bg-destructive/10 p-4 text-sm text-destructive">
					No fue posible cargar la capacidad instalada.
				</p>
			) : summary.isPending ? (
				...
				<div className="grid gap-4 sm:grid-cols-2">
					<MetricCard
						...
						metric={summary.data.metrics.installedCapacity}
					/>
					<MetricCard
						label="Cobertura de datos de capacidad"
						metric={summary.data.metrics.capacityDataCoverage}
					/>
				</div>
			)}
			<div className="mt-8 border-t border-border pt-6">
				<div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
					<div>
						...
						<p className="text-xs text-muted-foreground">
							{details.data
								? `${details.data.meta.total} aulas · capacidad y catálogo actuales`
								: 'Detalle paginado'}
						</p>
					</div>
							...
							<select
								...
								onChange={event => {
									if (
										...
										setCapacitySort(event.target.value);
								}}
							>
								{SORTS.map(item => (
				...
				{details.isError ? (
					<p className="rounded-xl border border-destructive/30 bg-destructive/10 p-4 text-sm text-destructive">
						No fue posible cargar el detalle de aulas.
					</p>
				) : outOfRange ? (
