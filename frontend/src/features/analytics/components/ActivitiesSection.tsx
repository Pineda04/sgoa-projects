...
const METRICS = [
	...
	{ key: 'reportedTeachers', label: 'Docentes con reporte' },
	{
		...
		label: 'Promedio por docente reportante',
	},
	{
		key: 'activeTeacherReportCoverage',
		label: 'Cobertura de docentes activos',
	},
] satisfies {
	key:
		| 'totalActivities'
...
const SORTS = [
	{ value: 'activityName:asc', label: 'Actividad A-Z' },
	{ value: 'activityName:desc', label: 'Actividad Z-A' },
	{ value: 'typeName:asc', label: 'Tipo A-Z' },
	{ value: 'typeName:desc', label: 'Tipo Z-A' },
	{ value: 'teacherName:asc', label: 'Docente A-Z' },
	{ value: 'teacherName:desc', label: 'Docente Z-A' },
	{ value: 'period:asc', label: 'Período ascendente' },
	{ value: 'period:desc', label: 'Período descendente' },
	{ value: 'progressLevel:asc', label: 'Progreso ascendente' },
	{ value: 'progressLevel:desc', label: 'Progreso descendente' },
] satisfies { value: ActivityDetailSort; label: string }[];

const registrationLabel = (value: boolean | null) =>
	value === null ? 'Sin estado' : value ? 'Registrada' : 'No registrada';

const createColumns = (
	canViewReports: boolean
): IResponsiveColumn<ActivityDetailRow>[] => [
	{
		key: 'activity',
		header: 'Actividad',
		render: (row: ActivityDetailRow) => (
			<div className="min-w-0 text-left">
				<p className="break-words font-semibold">{row.activityName}</p>
	...
	},
	{
		key: 'period',
		header: 'Período',
		render: (row: ActivityDetailRow) => row.period.label,
	},
	{
		key: 'centerDepartment',
		header: 'Centro y departamento',
		render: (row: ActivityDetailRow) => row.centerDepartment.label,
	},
	{ key: 'progressLevel', header: 'Nivel de progreso' },
	{
		key: 'isRegistered',
		header: 'Registro',
		render: (row: ActivityDetailRow) => registrationLabel(row.isRegistered),
	},
	{
...
export const ActivitiesSection = () => {
	const {
		...
		values,
		options,
	} = useAnalyticsFilters('activities');
	...
	if (!activityFilters)
		return (
			<p className="py-8 text-center text-sm text-muted-foreground">
				No hay una selección temporal autorizada para consultar.
			</p>
		);
	...
	const canViewReports =
		ability.can('read', 'reports') || ability.can('manage', 'reports');
	const columns = createColumns(canViewReports);
	const distribution = summary.data
		? breakdown === 'type'
			? summary.data.distributions.byType
			: breakdown === 'period'
				? summary.data.distributions.byPeriod
				: breakdown === 'center'
					? summary.data.distributions.byCenterDepartment
					: summary.data.distributions.byTeacher
		: [];
	const setBreakdown = (value: string) => {
		const nextValue: ActivityBreakdown =
			value === 'period' || value === 'center' || value === 'teacher'
				? value
				: 'type';
	...
	const exportRows = async () => {
		setIsExporting(true);
		try {
			const response = await analyticsApi.exportActivities({
				...activityFilters,
				metric: 'activities',
				sort: activitySort,
			});
			const temporal =
				activityFilters.periodId ?? activityFilters.year ?? 'seleccion';
			downloadBlob(
				response.data,
				`analytics-actividades-${temporal}.xlsx`
			);
		} catch {
			await genericAlert(
				'No fue posible exportar las actividades.',
				ESwalIcons.ERROR
			);
		} finally {
		...
		<div>
			<div className="mb-5">
				...
				<p className="mt-1 text-sm text-muted-foreground">
					Las actividades se leen desde los reportes de asignación y
					su tipo se toma del catálogo vigente. Esta sección muestra
					lo registrado, pero no gestiona ni muestra estados de
					revisión o aprobación del reporte.
				</p>
			</div>
			{summary.isError ? (
				<p className="rounded-xl border border-destructive/30 bg-destructive/10 p-4 text-sm text-destructive">
					No fue posible cargar los indicadores de actividades.
				</p>
			) : summary.isPending ? (
			...
			<div className="mt-8 border-t border-border pt-6">
				<div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
					<div>
						...
						<p className="text-xs text-muted-foreground">
							{details.data
								? `${details.data.meta.total} actividades · una fila por actividad`
								: 'Detalle paginado'}
						</p>
					</div>
						...
						<label className="flex items-center gap-2 text-sm font-semibold">
							...
							<select
								...
								onChange={event => {
									...
									if (sort) setActivitySort(sort.value);
								}}
							>
								{SORTS.map(item => (
				...
				{details.isError ? (
					<p className="rounded-xl border border-destructive/30 bg-destructive/10 p-4 text-sm text-destructive">
						No fue posible cargar el detalle de actividades.
					</p>
				) : outOfRange ? (
