...
export const useCreateAudioEquipmentMutation = () => {
	...
	return useMutation({
		mutationFn: (data: TCreateAudioEquipment) =>
			audioEquipmentsApi.createAudioEquipment(data),
		onSuccess: async () => {
...
export const useUpdateAudioEquipmentMutation = () => {
	...
	return useMutation({
		...
		onSuccess: (_, variables) => {
			...
			queryClient.invalidateQueries({
				queryKey: audioEquipmentsKeys.detail(variables.id),
			});
		},
	});
};
